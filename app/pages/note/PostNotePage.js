...
class PostNotePage extends Component {
    constructor(props) {
        super(props);
        var addressDS = new ListView.DataSource({
            rowHasChanged: (r1, r2) => {
                r1.addr !== r2.addr;
            }
        });
    ...
    _sendPhotos(noteId) {
        ...
        request('/notes/' + noteId + '/images', 'POST', JSON.stringify(data), this.state.token)
            .then((responseJson) => {
    ...
    _sendNote() {
        ...
        request('/user/notes/', 'POST', JSON.stringify(data), this.state.token)
            .then((responseJson) => {
                ...
                Toast.show('发送失败，请重试。', {duration: Toast.durations.SHORT, position: Toast.positions.CENTER});
                this.state.posting = false;
            })
            .catch((error) => {
                Toast.show('发送失败，请重试。', {duration: Toast.durations.SHORT, position: Toast.positions.CENTER});
        ...
        for (let i = 0; i <= rowIndex; i++) {
            let row = <View key={i}
                            style={{
                                flexDirection: 'row',
                                paddingVertical: 5
                            }}>{this._renderPhotosRow(photos, photosPerRow, i * photosPerRow)}</View>;
            photoRows.push(row);
        }
